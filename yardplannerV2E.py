...
class ConflictResolver:
    @staticmethod
    def resolve_12m_6m_conflict(area: YardArea, container: Container, available_positions: List[str]) -> List[str]:
        if container.size == 12:
            valid_positions = []
            for pos_key in available_positions:
                row = area.positions[pos_key]['row']
                if row % 2 == 0:
                    adjacent_6m_1 = f"{area.name}{row-1}{pos_key[-1]}"
                    adjacent_6m_2 = f"{area.name}{row+1}{pos_key[-1]}"
                    
                    adj1_usage = len(area.positions.get(adjacent_6m_1, {}).get('containers', []))
                    adj2_usage = len(area.positions.get(adjacent_6m_2, {}).get('containers', []))
                    
                    if adj1_usage == 0 and adj2_usage == 0:
                        valid_positions.append(pos_key)
    ...
    if import_file:
        try:
            ...
            if missing_cols:
                ...
                selected_area = st.selectbox("Select Area for Import Planning", planner.areas.keys())
                ...
                if st.button("🚀 Plan Imports", type="primary", key="plan_imports"):
                    with st.spinner(f"Planning {len(imports_df)} import containers in {selected_area}..."):
                        ...
                        import_placements = planner.optimize_import_layout(import_containers, area_config)
                        st.session_state.import_placements = import_placements['placement_map']
                        
                        placed_count = import_placements['total_placed']
                        ...
                        if import_placements['placement_map']:
                            import_df = pd.DataFrame.from_dict(import_placements['placement_map'], orient='index')
            ...
            if missing_cols:
                ...
                if st.button("🚀 Plan Exports", type="primary", key="plan_exports"):
                    with st.spinner("Planning export containers across areas..."):
                        ...
                        export_placements = planner.optimize_export_layout(export_containers, area_weight_config)
                        st.session_state.export_placements = export_placements['placement_map']
                        
                        placed_count = export_placements['total_placed']
                        ...
                        if export_placements['placement_map']:
                            export_df = pd.DataFrame.from_dict(export_placements['placement_map'], orient='index')
